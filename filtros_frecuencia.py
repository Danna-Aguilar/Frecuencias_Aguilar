...
indices= ((frecuencias> 1500) & (frecuencias < 1900))
...
pasa_altas[frecuencias < 2500] *= 0

ejes[1,3].plot(frecuencias, np.abs(pasa_altas), label = "Espectro filtrado, pasa altas")
ejes[1,3].legend()
ejes[1,3].set(xlabel = "Tiempo (s)", ylabel = "Amplitud")

pasa_altas_data = np.fft.irfft(pasa_altas)
ejes[1,2].plot(tiempos, pasa_altas_data, label = "Audio con pasa altas")
ejes[1,2].legend()
ejes[1,2].set(xlabel = "Tiempo (s)", ylabel = "Amplitud")
